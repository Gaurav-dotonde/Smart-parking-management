...
export default function AdminLots() {
  ...
  return <div className="admin-module-page">
    ...
    <section className="admin-data-card">
      {loading ? <div className="admin-empty-compact"><span className="admin-spinner" /> Loading parking locations…</div> : !visible.length ? <div className="admin-empty-compact">No parking locations match the selected filters.</div> : <div className="admin-responsive-table admin-lots-table-wrap"><table className="admin-lots-table"><thead><tr><th>Location</th><th>Address</th><th>Total Floors</th><th>Total Slots</th><th>Available</th><th>Status</th><th>Actions</th></tr></thead><tbody>{visible.map((lot) => <tr key={lot.id} className="admin-clickable-row" tabIndex={0} onClick={() => setViewing(lot)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); setViewing(lot); } }}><td><strong>{lot.name}</strong><small>{[lot.area, lot.city].filter(Boolean).join(', ') || 'Area not specified'}</small></td><td>{lot.address || lot.location}<small>{[lot.state, lot.pinCode].filter(Boolean).join(' ')}</small></td><td>{lot.totalFloors ?? 0} Floors</td><td>{lot.totalSlots ?? 0}</td><td>{lot.availableSlots ?? 0}</td><td><span className={`admin-status ${lot.archived ? 'maintenance' : lot.active ? 'active' : 'disabled'}`}>{lot.status || (lot.archived ? 'ARCHIVED' : lot.active ? 'ACTIVE' : 'INACTIVE')}</span></td><td><div className="admin-row-actions admin-lots-row-actions"><button onClick={(event) => { event.stopPropagation(); navigate(`/admin/slots?locationId=${lot.id}`); }}>View Slots</button><button onClick={(event) => { event.stopPropagation(); startEdit(lot); }} disabled={lot.archived}>Edit</button><button className="danger" onClick={(event) => { event.stopPropagation(); setConfirming(lot); }} disabled={lot.archived}>Delete</button></div></td></tr>)}</tbody></table></div>}
    </section>
